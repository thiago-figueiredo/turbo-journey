...
let response = await fetch("https://api-stage.twiagemed.net/v4/users", {
  headers: {
...
if (arg1 === "DRY") {
  ...
  const x = data
    .filter((member: Member) => pattern.test(member.email))
...
while (membersToDelete.length > 0) {
  ...
  response = await fetch(`https://api-stage.twiagemed.net/v4/users/${userID}`, {
    headers: {
      authorization: `JWT ${token}`,
    },
    method: "DELETE",
